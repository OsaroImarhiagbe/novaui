...
import { PartnerGoals } from "@/utils/data/data";
...
const SettingGoal = async (data:GoalUpdate) => {
  const response = await axios.post(`api/goal-service`,{ //need to figure out how to do goals for people in relationships
    goals:{
      title:data.goal.title
    }
  }) // need to add user_id this will come from state management
...
export default function PartnerScreen() {
  ...
  const handleSelectedGolas = useCallback((goal:string,checked:boolean) => {
  ...
  },[mutate,selectedGoals,sendMessage])

  useEffect(() => {
    sendMessage("The user's relationship status is 'in a relationship'.")
  ...
  return (
    ...
    >
      ...
      >
        ...
        Personalize your couples experience. ✨
        </h1>
        <p className="text-2xl font-semibold transition-colors">
        🎯 What would you love to achieve as a couple?
        </p>
      </motion.div>
      <motion.div
        variants={STAGGER_CHILD_VARIANTS}
      ...
      >
        {PartnerGoals.map((goal,index) => {
          return (
            <div key={index} className="flex items-center space-x-2 m-6">
             <Checkbox
             checked={selectedGoals.includes(goal)}
             onCheckedChange={(checked) => handleSelectedGolas(goal,!!checked)}
             />
             <label
              htmlFor="terms"
